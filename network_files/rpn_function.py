...
class AnchorsGenerator(nn.Module):
    ...
    def forward(self, image_list, feature_maps, center):
        # type: (ImageList, List[Tensor], bool) -> List[Tensor]
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head,
        ...
        self.whole_features = [2, 3, 4]
        self.query_layer_train = [1, 2]
        self.value_features = [0, 1]
        self.small_obj_scale = [[0, 32], [0, 64]]
        self.small_center_dis_coeff = [1., 1.]
        self.query_loss_weights = [10., 10.]
        self.query_loss_gammas = [1.2, 1.2]
        self.context = 2
    ...
    def get_anchor_center_min_dis(self, box_centers: torch.Tensor, anchor_centers: torch.Tensor):
        """
        Args:
            box_centers: [N, 2]
            anchor_centers: [M, 2]
    ...
    def get_query_gt(self, small_anchor_centers, targets):
        ...
        for lind, anchor_center in enumerate(small_anchor_centers):
            ...
            for target_per_image in targets:
                target_box_scales = torch.sqrt((target_per_image['boxes'][:, 2] - target_per_image['boxes'][:, 0]) * (target_per_image['boxes'][:, 3] - target_per_image['boxes'][:, 1]))

                small_inds = (target_box_scales < self.small_obj_scale[lind][1]) & (
                            target_box_scales >= self.small_obj_scale[lind][0])
                small_boxes = target_per_image['boxes'][small_inds]
                small_center = torch.stack([(small_boxes[:, 0] + small_boxes[:, 2])/2, (small_boxes[:, 1] + small_boxes[:, 3])/2], dim=1)
                center_dis, minarg = self.get_anchor_center_min_dis(small_center, anchor_center)
    ...
    def _query_anchor(self, query_logits, objectness_val, pred_bbox_deltas_val, anchors, feature_value):

        N, _, qh, qw = query_logits.size()
        assert N == 1
        prob = torch.sigmoid_(query_logits).view(-1)
        pidxs = torch.where(prob > 0.12)[0]  # .float()
        ...
        anchors = anchors.view(-1, 3, 4)
        objectness_val = objectness_val.view(-1, 3, 1)
        pred_bbox_deltas_val = pred_bbox_deltas_val.view(-1, 12, 1)
        ys = objectness_val[inds].view(-1, 1)
        xs = pred_bbox_deltas_val[inds].view(-1, 4)
        selected_anchors = anchors[inds].view(-1, 4)
        return ys, xs, inds, selected_anchors
    ...
    def forward(self,
        ...
        # whole_feature = [features[x] for x in self.whole_features]
        query_feature = [features[x] for x in self.query_layer_train]
        val_feature = [features[x] for x in self.value_features]
        # features = [features[x] for x in self.whole_features]
        ...
        anchors = self.anchor_generator(images, features, False)
        # numel() Returns the total number of elements in the input tensor.
        # 计算每个预测特征层上的对应的anchors数量
        num_anchors_per_level_shape_tensors = [o[0].shape for o in objectness]
        num_anchors_per_level = [s[0] * s[1] * s[2] for s in num_anchors_per_level_shape_tensors]
        # val_anchor = [anchors[0][x] for x in self.value_features]
        # objectness_val = [objectness[x] for x in self.value_features]
        # pred_bbox_deltas_val = [pred_bbox_deltas[x] for x in self.value_features]
        # objectness = [objectness[x] for x in self.whole_features]
        # pred_bbox_deltas = [pred_bbox_deltas[x] for x in self.whole_features]
        # 调整内部tensor格式以及shape
        ...
        #
        #
        # for i in range(len(val_feature)-1, -1, -1):
        #     selected_objectness, selected_pred_bbox_deltas, inds, selected_anchors = self._query_anchor(query_logits[i], objectness_val[i], pred_bbox_deltas_val[i], val_anchor[i], val_feature[i])
        #     query_anchors.append(selected_anchors)
        #     select_obj.append(selected_objectness)
        #     select_pred.append(selected_pred_bbox_deltas)
        # select_obj = torch.cat((select_obj[0], select_obj[1]), 0)
        # select_pred = torch.cat((select_pred[0], select_pred[1]), 0)
        # anchors[0] = query_anchors + [anchors[0][x] for x in self.whole_features]
        # num_anchor_val = [query_anchors[0].shape[0], query_anchors[1].shape[0]]
        # num_anchors_per_level = num_anchor_val + [num_anchors_per_level[x] for x in self.whole_features]
        #
        # objectness = torch.cat((select_obj, objectness), 0)
        # pred_bbox_deltas = torch.cat((select_pred, pred_bbox_deltas), 0)
        anchors = [torch.cat(anchors_per_image) for anchors_per_image in anchors]
        # batch_size
        ...
        if self.training:
            ...
            # query_center = self.anchor_generator(images, query_feature, True)
            # gt_query = self.get_query_gt(query_center, targets)
