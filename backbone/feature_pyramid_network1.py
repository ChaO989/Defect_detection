...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(self, in_channels_list, out_channels, extra_blocks=None):
        ...
        for in_channels in in_channels_list:
            ...
            eca_block_module = eca_layer(in_channels, k_size=5)
            self.inner_blocks.append(inner_block_module)
            self.layer_blocks.append(layer_block_module)
            self.eca.append(eca_block_module)
    ...
    def get_result_from_eca_blocks(self, x: Tensor, idx: int) -> Tensor:
        """
        This is equivalent to self.layer_blocks[idx](x),
        but torchscript doesn't support this yet
        """
        num_blocks = len(self.eca)
        if idx < 0:
            idx += num_blocks
        i = 0
        out = x
        for module in self.eca:
            if i == idx:
                out = module(x)
            i += 1
        return out
    ...
    def forward(self, x: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        results.append(self.get_result_from_eca_blocks(last_layer, -1))

        for idx in range(len(x) - 2, -1, -1):
            inner_lateral = self.get_result_from_inner_blocks(x[idx], idx)
            inner_lateral = self.get_result_from_eca_blocks(inner_lateral, idx)
            feat_shape = inner_lateral.shape[-2:]
            inner_top_down = F.interpolate(last_inner, size=feat_shape, mode="nearest")
            last_inner = inner_lateral + inner_top_down
            last_layer = self.get_result_from_layer_blocks(last_inner, idx)
            results.insert(0, self.get_result_from_eca_blocks(last_layer, idx))
